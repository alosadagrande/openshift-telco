...
class ocp(Cluster):
    ...
    option_list = [
        ...
        ('api-url', '', 'API URL to use for oc authorization'),
    ...
    def oc_cmd(self):
        if not self._oc_cmd:
            ...
            if self.get_option('kubeconfig'):
                self._oc_cmd += " --kubeconfig %s" % self.get_option('kubeconfig')
    ...
    def _attempt_oc_login(self):
        ...
        _res = self.exec_primary_cmd(
            self.fmt_oc_cmd("login --insecure-skip-tls-verify=True --token=%s %s"
                            % (self.token, self.api-urli))
        )
